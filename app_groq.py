...
.stat-number { font-size: 2.4rem; font-weight: 800; color: #F59E0B !important; }
...
try:
    ...
    st.error(f"تعذّر الاتصال بـ Groq API. تأكد من ضبط GROQ_API_KEY في Secrets. {e}")
    st.stop()
...
with st.sidebar:
    ...
    st.markdown("يمكنك دعم هذا المشروع عبر **بريدي موب** أو **CCP**:")
    st.markdown("""
<div class="ccp-box">
...
with col1:
    ...
    level = st.selectbox("المستوى", LEVELS, label_visibility="collapsed")
    st.markdown('</div>', unsafe_allow_html=True)

with col2:
    st.markdown('<div class="card"><h3>📅 الفصل الدراسي</h3>', unsafe_allow_html=True)
    trimester = st.selectbox("الفصل", TRIMESTERS, label_visibility="collapsed")
    st.markdown('</div>', unsafe_allow_html=True)

with col3:
    st.markdown('<div class="card"><h3>📚 المادة المقررة</h3>', unsafe_allow_html=True)
    subject = st.selectbox("المادة", SUBJECTS, label_visibility="collapsed")
    st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<div class="card"><h3>📝 عناوين الدروس المُدرَّسة في هذا الفصل</h3>', unsafe_allow_html=True)
...
def build_prompt(level, trimester, subject, lessons):
    ...
    lang_note = (
        ...
        if is_french else
        "يجب أن تكون جميع نصوص الاختبار مشكولةً شكلاً تاماً وصحيحاً بالحركات."
    )
    header_note = (
        "استخدم الترويسة: République Algérienne Démocratique et Populaire / Ministère de l'Éducation Nationale."
        if is_french else
        "استخدم الترويسة: الجمهورية الجزائرية الديمقراطية الشعبية / وزارة التربية الوطنية."
    )
    return f"""
...
المعايير:
1. {header_note}
2. كل اقتراح يحتوي: ترويسة رسمية كاملة، أسئلة موزعة على 10 نقاط، دليل التصحيح النموذجي.
3. {lang_note}
4. تنوّع الأسئلة: فهم، تطبيق، إنتاج، تصحيح أخطاء.
5. في أسفل كل اقتراح أضف:
   ملاحظات السيد المفتش التربوي: ............................................
   توقيع الأستاذ: .................................  التاريخ: .................

افصل بين الاقتراحات بسطر يحتوي فقط على: === الاقتراح الأول === أو === الاقتراح الثاني === أو === الاقتراح الثالث ===
...
if generate:
    if not lessons.strip():
        ...
        steps = [
            ...
            (25, "📡  تم الاتصال! جاري تحليل المنهج..."),
            (45, "🧠  الذكاء الاصطناعي يصوغ الأسئلة..."),
            (65, "✍️  جاري مشكلة النصوص..."),
            (80, "📋  جاري بناء دليل التصحيح..."),
            (92, "💅  اللمسات الأخيرة..."),
        ]
...
if st.session_state.results:
    ...
    st.download_button(
        label="📦  تحميل الحزمة الكاملة — ملف Word واحد",
        data=docx_all,
